...
def manual(q, qdot, l, m, I, g):
    v_2 = l[0] ** 2 * qdot[0]**2 + 0.25 * l[1]**2 * (qdot[0] + qdot[1])**2 + l[0] * l[1] * np.cos(q[1]) * qdot[0] * (qdot[0] + qdot[1])
    ...
    print('w_2 :', w_2)
    """
    T = 0.5 * v_1 * m[0] + 0.5 * v_2 * m[1] + 0.5 * I * w_1**2 + 0.5 * I * w_2**2
    V = m[0] * g * l[0] * 0.5 * np.sin(q[0]) + m[1] * g * (l[0] * np.sin(q[0]) + 0.5 * l[1] * np.sin(q[0] + q[1]))
    return T - V
...
def create3DDynamics(n):
    ...
    I_jex  = np.zeros((9, 2))
    com_ex = np.array([[0.0, 0.0], [l_ex[0]/2.0, l_ex[1]/2.0], [0, 0]])
    ...
    g_ex = np.array([-9.8])
    q_ex = np.array([-1.5708, 1.5708])
    q_ex = np.array([0.0, 0.0])
    qdot_ex = np.array([0.0, 0.0])
    ...
    for i in range(n):
        ...
        """
        test = ca.Function("test", [q, qdot, com, m, I_j, g, axis_j, off_j], [r_0ic])
        print("test : ", np.transpose(np.round(test(q_ex, qdot_ex, com_ex, m_ex, I_jex, g_ex, axis_jex, off_jex))))
        """
    ...
    L_fun = ca.Function("L", [q, qdot, com, m, I_j, g, axis_j, off_j], [L])
    """
    print("L : ", L_fun(q_ex, qdot_ex, com_ex, m_ex, I_ex, g_ex, axis_jex, off_jex))
    print("man : ", manual(q_ex, qdot_ex, l_ex, m_ex, I_ex[2, 2], -g_ex[0]))
    """
